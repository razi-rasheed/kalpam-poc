...
for (const dir of PACKAGES) {
  ...
  let exists = false;
  try { execFileSync("npm", ["view", `${name}@${version}`, "version"], { stdio: "ignore" }); exists = true; } catch {} // any error (incl. "not found" or registry/auth failure) → fall through to publish, which fails with a clear error if the registry is unreachable
